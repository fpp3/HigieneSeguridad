...
resto de las naves y posibilitando la evacuación segura del personal antes del colapso estructural.
...
incombustibles; siendo el almacenamiento industrial típicamente clasificado como Riesgo 3, muy combustible, o Riesgo 4,
combustible), determina la resistencia al fuego mínima de muros y los requerimientos de ventilación y extinción.

\subsection{Unidades de Ancho de Salida (UAS) y Medios de Evacuación}

El dimensionamiento de las vías de escape responde a criterios deterministas basados en el concepto de Unidades de Ancho
de Salida (UAS). Una UAS representa el espacio libre requerido para que circule una hilera de personas en marcha
ordenada. La reglamentación fija que las dos primeras unidades equivalen a $0.55\m$ de ancho útil cada una (totalizando un
ancho mínimo de acceso de $1.10\m$), mientras que cada unidad adicional subsecuente añade $0.45\m$. Ninguna puerta de
emergencia o pasaje podrá tener un ancho libre inferior a $0.90\m$.

Para calcular la población teórica ($N$) a evacuar, se divide la superficie del sector por el factor de ocupación
($f_o$), conforme a la ecuación \ref{eq.cap4:ocupantes}:
...
El factor $f_o$ depende del destino edilicio (e.g., $f_o = 16\m^2/\text{persona}$ en naves industriales mecánicas,
$f_o = 8\m^2/\text{persona}$ en oficinas, y $f_o = 40\m^2/\text{persona}$ en depósitos generales). Determinado $N$, el
número requerido de unidades de ancho de salida ($n$) se calcula mediante la ecuación \ref{eq.cap4:uas}:
...
Toda fracción igual o superior a $0.5$ se redondea al entero superior más próximo. Por disposición legal, el ancho
mínimo permitido para cualquier salida colectiva corresponde a $2\UAS$ ($1.10\m$).

En vías de evacuación que incluyan escaleras, los peldaños deben responder a la fórmula ergonómica de paso de Blondel,
...
Donde la alzada ($a$) no puede superar los $0.18\m$ y la pedada ($p$) no debe ser inferior a $0.26\m$. La distancia
máxima desde el punto más desfavorable de trabajo hacia una caja de escaleras o salida al exterior no puede exceder los
$40\m$. Si el cálculo arroja que $n \ge 4$, el flujo de personas debe dividirse obligatoriamente en medios de escape
independientes ($ME$), según la ecuación \ref{eq.cap4:escape_indep}:
...
inmuebles en el territorio nacional. En materia de seguridad contra incendios destacan dos capítulos:
