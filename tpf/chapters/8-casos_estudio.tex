\chapter{Análisis Forense de Siniestros y Casos de Estudio Reales}
\label{sec.cap8:casos_estudio}

El estudio de siniestros de gran escala constituye una herramienta pedagógica y pericial indispensable en la formación
de los ingenieros. Una vez desarrollados los fundamentos termodinámicos de la combustión (Capítulo
\ref{sec.cap2:fenomeno_fisico}), los efectos toxicológicos sobre las personas (Capítulo \ref{sec.cap3:ser_humano}), las
exigencias de cálculo reglamentario (Capítulo \ref{sec.cap4:aspectos_legales}), las tecnologías de detección (Capítulo
\ref{sec.cap5:sistemas_deteccion}), los métodos de extinción (Capítulo \ref{sec.cap6:sistemas_extincion}) y las pautas
de capacitación y brigadas (Capítulo \ref{sec.cap7:capacitacion_emergencias}), resulta posible abordar con rigor analítico
las causas raíces, los modos de falla de las barreras de protección y las lecciones operativas derivadas de siniestros
reales documentados.
...
\section{Matriz Comparativa de Lecciones Forenses}
\label{sec.cap8:matriz_forense}

En el Cuadro \ref{tab.cap8:matriz_casos} se presenta una síntesis comparativa de los tres casos periciales analizados,
destacando el origen de la falla, las debilidades observadas en los sistemas de protección y las prescripciones
ingenieriles requeridas para prevenir su recurrencia.

\begin{table}[!ht]
\centering
\caption{Matriz comparativa de siniestros industriales documentados y lecciones de ingeniería.}
\label{tab.cap8:matriz_casos}
\small
\begin{tabular}{|>{\raggedright\arraybackslash}p{2.1cm}|>{\raggedright\arraybackslash}p{2.2cm}|>{\raggedright\arraybackslash}p{2.3cm}|>{\raggedright\arraybackslash}p{3.1cm}|>{\raggedright\arraybackslash}p{3.5cm}|}
\hline
\textbf{Siniestro} & \textbf{Clase de Fuego} & \textbf{Causa Primaria} & \textbf{Fallas en Barreras} & \textbf{Medidas Correctivas Clave} \\
\hline
\textbf{Iron Mountain} (Bs.~As., 2014) & Clase A (celulosa compacta) & Focos múltiples; carga térmica extrema & Rociadores inoperativos sin presión; estructura de acero desnuda sin ignifugado & Auditoría de bombas y válvulas; protección pasiva $F120/F180$; zonas de seguridad perimetral. \\
\hline
\textbf{Edesur Caballito} (Bs.~As., 2024) & Clase B y C (aceite mineral en MT) & Rotura de manguera a presión en filtrado de aceite & Ausencia de contención de derrames; cables con aislación propagadora de llama & Espuma AFFF; fosa colectora con apagallamas; conductores libres de halógenos (AEA 90364). \\
\hline
\textbf{Renesas Electronics} (Japón, 2021) & Clase C y A (sobrecorriente en cleanroom) & Efecto Joule por sobrecorriente en equipo galvánico & HVAC recirculó humos ácidos en sala limpia; falta de supresión gaseosa & Detección ASD/VESDA; corte de compuertas HVAC; inundación por agente limpio FK-5-1-12. \\
\hline
\end{tabular}
\end{table}
